...
<template>
   <section>
        <el-row  class="toolbar">
            <el-col :span="3">
                <el-button type="primary" size="medium" icon="el-icon-circle-plus" @click="addReturn">退还</el-button>
            </el-col> 
            <el-col  :span="21">    
        ...
        <el-table :data="dataList"  style="width: 100%"  border v-loading="visible.loading">
            ...
            <el-table-column   label="供应商"   prop="supplier.supplier_name || '暂无数据'" > </el-table-column>
            <el-table-column   label="退还数量"   prop="warehouse_material_return_number">  </el-table-column>
            <el-table-column   label="编辑"   prop=""  width="200"> 
                <template slot-scope="scope">
                    ...
                    <el-button size="small" icon="el-icon-delete" @click="remove(scope.row.warehouse_material_return_id)" type="danger">删除</el-button>
                </template>    
            </el-table-column>
        ...
        <el-dialog :visible.sync="visible.addForm" :close-on-click-modal="false" width="60%">
            <span slot="title" class="el-dialog__title" >退还登记</span>
            <el-container>
                <el-main>
                        ...
                        <el-col :span="8">
                            <div>
                                <el-input placeholder="请输入内容" v-model="people" @click.native="selectPeople()">
                                    <template slot="prepend" >退还人员</template>
                                </el-input>
                            </div>
                        </el-col>
                        <el-col :span="8">
                            <div>
                                <el-input placeholder="请输入内容" v-model="workshop"  @click.native="selectWorkshop()">
                                    <template slot="prepend">生产车间</template>
                                </el-input>
                            </div>
                        </el-col>
                        <el-col :span="8">
                            <div>
                                <span class="time">退还时间</span>
                                <el-date-picker
                                    v-model="time"
                                    type="datetime" 
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th style="width: 10%;">退还仓库</th>
                                <th style="width: 10%;">退还数量</th>
                        ...
                        <tbody>
                            <tr v-for="(value,index) in selectMaterialList"  :key="index">
                                ...
                                <td>
                                    <el-select  placeholder="请选择仓库" size="medium"  v-model="selectMaterialList[index].warehouse_return_id">
                                        <el-option  v-for="(val,index) in warehouseName" 
                                        :value="val.warehouse_id"
                                        :key="index" 
                                        :label="val.warehouse_name" 
                                ...
                                <td>
                                    <el-input  size="medium"  placeholder="请输入内容(必填)"  type = "number" min = 0 
                                    :value = 'value.warehouse_material_return_number' @blur="test(value.warehouse_material_return_number,index)"
                                    v-model="selectMaterialList[index].warehouse_material_return_number"></el-input>
                                </td>
            ...
            <el-container>
                <el-main>
                    ...
                    <el-table :data="materialList"  style="width: 100% ;padding:3px 0"  border>
                        ...
                        <el-table-column   label="出库仓库"   prop="warehouseOut">  </el-table-column>
...
<script>
    ...
    import {getWarehouseBasic,queryMaterials,removeMaterialReturns,queryMaterialInventory,saveMaterialReturns} from '../../api/api' 
        ...
        methods: {
            ...
            getBasic () {
                ...
                getWarehouseBasic(this.req).then((data) => {
                    if (data.status === 0) {
                        this.dataList = data.map.materialReturns
                        this.lines = data.map.materialReturnLines
            ...
            addSubmit(){
                var addStatus = true
                if(this.people === "" || this.workshop === "" || this.time === ""){
                        this.$message({
                                type: 'danger',
                ...
                if(this.selectMaterialList.length !== 0){
                    ...
                    this.selectMaterialList.forEach((val,index)=>{
                        if(val.warehouse_return_id == "" || val.warehouse_return_id == null || val.warehouse_return_id == undefined ){
                            this.$message({
                                type: 'warning',
                                message: '请选择退还仓库'
                            });
                        ...
                        if(val.warehouse_material_return_number == "" || val.warehouse_material_return_number == null || val.warehouse_material_return_number == undefined ){
                            this.$message({
                                type: 'warning',
                                message: '请填写正确的退还数量'
                            });
                        ...
                        if(val.warehouse_material_return_number <= 0 ){
                            this.$message({
                                type: 'warning',
                                message: '退还数量不能小于等于零'
                            });
                        ...
                        const temp = `${val.warehouse_material_id}:${val.warehouse_material_batch}:${val.warehouse_material_return_number}:${val.supplier_id}:${val.warehouse_out_id}:${val.warehouse_return_id}`
            ...
            lookDetail (row) {//
                ...
                '出库详情', {
                    table: {
                    props: ['物料名称:','物料批次:','物料类型:', '物料规格:', '物料型号:', '单位:', '退还数量:', '供应商:','出库仓库:','退还仓库:','退还车间', '退还人员:','退还时间:', '退还原因:'],
                    values: [row.material.warehouse_material_name, row.warehouse_material_batch, row.material.warehouse_material_type_name, row.material.warehouse_material_standard,
                        row.material.warehouse_material_model, row.material.warehouse_material_units,row.warehouse_material_return_number,'暂无数据',row.warehouseOut,row.warehouseReturn, row.workshop?row.workshop.role_workshop_name:"暂无数据",
                        row.staff?row.staff.role_staff_name:"暂无数据", utils.formatDate.format(new Date( row.warehouse_material_return_time), 'yyyy-MM-dd hh:mm:ss'),row.warehouse_material_return_reason]
                    }
                })
            },
            //  获取物料列表
            getMaterials () {
                this.moduleReq.type = "return"
                queryMaterialInventory(this.moduleReq).then((data) => {
                    if (data.status === 0) {
                        this.materialList = data.map.materialOuts
                        this.materialLines = data.map.materialOuts.length
                    }else{
                        this.materialList = []
            ...
            addMaterial(val){
                ...
                temp.warehouseOut = val.warehouseOut
                temp.warehouse_out_id = val.warehouse_out_id
                temp.warehouse_return_id = val.warehouse_return_id
                temp.warehouse_material_return_number = ""
            ...
            selectWorkshop(index){//
                this.$mesWorkShop.show('', {
                    ...
                    this.workshop = res.rows[0].role_workshop_name
                    this.workshopId = res.rows[0].role_workshop_id
                })
            },
            ...
            test(value,index){
                if(value <= 0){
                    this.$message.error({message: '退还数量不能小于等于零'});
                    this.selectMaterialList[index].warehouse_material_return_number = ""
                }
