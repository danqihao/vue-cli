<!-- 刘川中 2018.3.28 -->
<template>
  <el-dialog :title="title || '产品型号' " :visible.sync="visible" class="mes-dialog" width="45%">
    ...
    >
      ...
      <el-table-column prop="product_model_genre" label="产品型号" sortable></el-table-column>
      <el-table-column prop="product_model_number" label="产品编号" sortable></el-table-column>
      <el-table-column prop="product_model_describe" label="备注" width="200" sortable></el-table-column>
    </el-table>
    <!--产品类型列表 end-->
...
<script>
  ...
  import {getProductModelList} from '../../../api/api'
  export default {
    name: 'product-model',
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        getProductModelList(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.lines
            this.list = data.map.productType
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.product_model_id === this.selectedIds[0]) {
              this.$nextTick(() => {
