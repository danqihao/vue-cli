<!-- 刘川中 2018.4.3 -->
<template>
  <el-dialog :title="title || '选择工序' " :visible.sync="visible" class="mes-dialog" width="40%">
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        queryWorkstageBasics(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.count
            this.list = data.map.workstageBasicsList
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.workstage_basics_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.workstage_basics_id === this.selectedIds[0]) {
              this.$nextTick(() => {
