<!-- 刘川中 2018.4.3 -->
<template>
  <el-dialog :title="title || '选择仓库' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="warehouse_name" label="仓库名称" sortable></el-table-column>
      <el-table-column prop="warehouse_site" label="仓库地址" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
...
<script>
  ...
  import { getWarehouseList } from '../../../api/api'
  export default {
    name: 'warehouse',
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        getWarehouseList(para).then((data) => {
          this.filters.total = data.map.warehouse.length
          this.list = data.map.warehouse
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.warehouse_id === this.selectedIds[0]) {
              this.$nextTick(() => {
