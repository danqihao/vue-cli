...
const WareHouse = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, WareHouse.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
WareHouse.setDefaults = defaults => {
  WareHouse.defaults = defaults
}

// 显示弹框
WareHouse.show = (title, options) => {
  ...
  return WareHouse(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
